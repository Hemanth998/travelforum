...
router.post(
  ...
  [
    ...
    [
      ...
      check('places', 'please select atlease one place').not().isEmpty()
    ]
  ],
  async (req, res) => {
    ...
    try {
      ...
      if (isExist) {
        return res.status(400).json({
          errors: [
            {
              msg: 'Hii!!, A post with similar or same title already exists, theres no unique slug for the title, try changing few characters for the title for now, we are working to generate unique slugs for each post, Thanks!!'
            }
          ]
        });
      }
...
router.get('/getPostsByPlace/:placeID', async (req, res) => {
  try {
    const posts = await Post.find({
      places: { $elemMatch: { place: req.params.placeID } }
    }).sort({ date: -1 });
    res.send(posts);
  } catch (err) {
    res.status(500).send('Internal Server error');
